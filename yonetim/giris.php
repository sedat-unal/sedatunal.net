    ...
    <section class="login-content">
      <div class="logo">
        ...
        if ($_POST){
            ...
            if (!$eposta || !$sifre){
                echo '<div class="alert alert-danger" role="alert">Lütfen bilgileri eksiksiz giriniz.</div>';
            }else{
                if (!filter_var($eposta, FILTER_VALIDATE_EMAIL)){
                    echo '<div class="alert alert-danger" role="alert">E-postanızı hatalı formatta girdiniz.</div>';
                }else {
                    $girisyap = $db->prepare("SELECT * FROM yoneticiler WHERE yonetici_eposta = :e 
                      AND yonetici_sifre = :s LIMIT :lim");
                    ...
                    if ($girisyap->rowCount()){
                        ...
                        $songiris = $db->prepare("UPDATE yoneticiler SET son_ip = :i,son_tarih = :t WHERE yonetici_id = :id");
                        $songiris->execute([':i' => IP(), ':t' => date('Y-m-d H:i:s'), ':id' => $row->yonetici_id]);
                        $adminid = $row->yonetici_id.IP();
                        $kripto  = sha1(md5($adminid));

                        $_SESSION['oturum'] = $kripto;
                        $_SESSION['id']     = $row->yonetici_id;
      ...
      <div class="login-box">
        <form class="login-form" action="" method="POST">
          ...
          <div class="form-group">
            ...
            <input name="eposta" class="form-control" type="text" placeholder="E-posta" autofocus>
          </div>
          <div class="form-group">
